...
# --> average 5possibilities/s for the iterative -i option
...
def parse_cmdline()
  OptionParser.new do |opts|
    ...
    opts.on("-i", "--iter_cost_dec"){set_iter()}    
    opts.on("-l", "--slack"){set_slack()}    
  end.parse!
...
class AlgoExhaustive
  ...
  min =Cost::new(999999999999999,9999999999999999999)
  minset = Array.new
    histogram ={}
    for z in 0..maxInt-1
      chooser[0] +=1
      ...
      mycost = thisCost.execute()
        if(mycost.less_than(min))
          minset.clear
          minset.push(treeSet)
          min = mycost
        elsif(mycost.equal_to(min))
          minset.push(treeSet)
        end      
...
#-------------------------------------------------------------------------
# Standard Dynamic Programming Algorithm - returns set of results
#-------------------------------------------------------------------------

class AlgoStandardDPSet
  ...
  def execute( part )
    ...
    if ( part.length == 1 )
      tree_set = []
      t = AssemblyTree::new_primitive(part)
      tree_set.push(t)
      return tree_set
    end
    ...
    for i in 0..(part.length-2)
      ...
      left_tree_set  = execute( left_part )
      right_tree_set = execute( right_part )
   
      tree_sets[i] = AssemblyTree::new_compound_set( left_tree_set, right_tree_set )
    end
    min_tree_set = Array.new
    # Find minimum cost assembly tree
    min_tree = AssemblyTree::new_primitive("k")
    min_tree.cost = Cost::new(9999999999999999,99999999999999999)
    ...
    tree_sets.each do |trees|
        for i in 0 .. trees.length-1
          if ( trees[i].cost < min_tree.cost )
            ...
            min_tree = trees[i]
          elsif( trees[i].cost ==min_tree.cost )
            min_tree_set = min_tree_set.push(trees[i])
          end
        end
    end
...
#part may be an array, always to_s
  def execute( part, max_stages, printStuff, makeFig)
    ...
    if ( @memoized_results.has_key?(part_stages) )
      t = @memoized_results[part_stages]
      tree_set=[]
      tree_set.push(t)
      return tree_set
    end
    ...
    for i in 0..(part.length-2)
      ...
      left_part  = part[ 0   .. i ]
      right_part = part[ i+1 .. part.length-1 ]
   #puts "looking at #{left_part}, #{right_part}"
      ...
      for i in 0 .. trees.length-1
        ...
        if($sharing)
          if(trees[i].cost.less_than( min_tree.cost, max_stages ))
            ...
            min_tree_set = min_tree_set.push(trees[i])
            min_tree = trees[i]
            #puts "new min! #{min_tree} cost: #{min_tree.cost}"
          elsif(trees[i].cost.equal_to( min_tree.cost, max_stages ))
            min_tree_set = min_tree_set.push(trees[i])
            #puts "new eqal! #{trees[i]} cost: #{trees[i].cost}"
          else
            #puts "new eqal! #{trees[i]} cost: #{trees[i].cost}"
          end     
    ...
    #puts "for part: #{part} #{min_tree_set[0].cost} treeset: #{min_tree_set}"
    return min_tree_set
...
#part may be an array, always to_s to access hash
  def execute( part, max_stages, printStuff, makeFig)
...
argGP = Array.new
  ...
  if($exhaustive && num_i ==0)
    stdDPSet = []
    goalparts.each do |goalpart|
      algo_setCross = AlgoStandardDPSet::new
      stdDPSet.push(algo_setCross.execute(goalpart))
    end
    puts "counting costs in exhaustive cross product"
    count_ex_cost(stdDPSet)
    puts "done ex"
    num_i = num_i+1
  end
    ...
    if($iter_cost_dec)
      algo_space = AlgoSDPSpace::new(sharing, lib)
      sDSpace_trees = []
      k = 0
      m = 1
      goalparts.each do | goalpart |
        sDSpace_trees[k] = algo_space.execute(goalpart, max_stages, printStuff, makeFig)
        m = m*sDSpace_trees[k].length
        puts "m is now: #{m} (*#{sDSpace_trees[k].length})"
        k = k+1
        #puts "#{goalpart} sdspace: #{sDSpace_trees[goalpart].length}  trees: #{sDSpace_trees[goalpart]}"
      end
      count_ex_cost(sDSpace_trees, final_trees)
    end
...
#-------------------------------------------------------------------------
# Arg: takes an array of array of assembly trees (A, B,...)
# Does: counts the cost of each assembly set resulting from the cross product AxBxC...
# final_trees -> chosen trees
# sDSpace -> array of arrays of assembly solutions to each goal part
#-------------------------------------------------------------------------

def count_ex_cost(sDSpace_trees, final_trees={})
  puts "starting count_ex_cost"
     #go through sDSpace_trees, choose one from each, add final_trees (chosen trees), calc cost
      max = 1
      bases = Array.new
      chooser = Array.new
      i=0
      sDSpace_trees.each do |treeset|
        max = max*treeset.length
        bases[i] = treeset.length
        chooser[i] = 0
        i=i+1
      end
      #puts "running iter_cost_dec for #{max} combinations"
      histogram ={}
      min =Cost::new(999999999999999,9999999999999999999)
      minset = Array.new
      for i in 0 ... max
        chooser[0] +=1
        for x in 0 ... bases.length
          if(chooser[x]%bases[x]==0 && chooser[x] !=0)
            chooser[x] = 0
            if(x+1<bases.length)
              chooser[x+1] += 1
        ...
        treeSet = {}
        for y in 0 ..chooser.length-1
          treeSet[y] =sDSpace_trees[y][chooser[y]]
        end
        #if sDSpace is too large, randomly pick a few tree sets to eval, discard the rest
        max_sec = 3000.0
        run_calc = true
        if(max > max_sec)
          if(rand(0) > max_sec/max)
            run_calc = false
          end
        end
        #puts "max #{max} div#{max_sec/max}"
        if(run_calc)
            treeSet = treeSet.merge(final_trees)
            #puts "calculating exhaustive for parts: #{@parts} and final: #{@final_trees} --> #{treeSet}"
            #print treeSet.to_s, "\t"
