...
class AlgoSharedDP
  ...
  def execute( part, max_stages, printStuff, makeFig)
    ...
    if ( @lib.has_key?(part) )
      ...
      if(printStuff)
        puts "  #{part.ljust(10)} #{tree.cost.to_s.ljust(10)} #{tree.to_s}"
      end
      return tree
    end
   ...
   if(makeFig)
      #puts "Makefig: #{rand(min_tree_set.length)}"
...
argGP = Array.new
  ...
  algo_exhaustive = AlgoExhaustive::new(goal_parts)
