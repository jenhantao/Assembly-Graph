 ...
 * @author vvasilev
 */
public class NewSDSAlgorithm {

    public HashMotifs _motifs;
    private ArrayList<CompositePart> _requiredParts;

    public NewSDSAlgorithm() {
     ...
     */
    public ArrayList<Tree> createAsmTreeMultipleGoalParts(ArrayList<CompositePart> goalPartsOrig, ArrayList<CompositePart> requiredPartsOrig, HashMem hashPartLibrary) {
        ArrayList<CompositePart> goalParts = (ArrayList<CompositePart>) goalPartsOrig.clone();
        _requiredParts = (ArrayList<CompositePart>) requiredPartsOrig.clone();
...
//        }
    ...
    public Tree createAsmTreeSingleGoalPart(CompositePart goalPart, HashMem hashMem, int slack, HashSharing hashSharing) {
        ...
        for (int start = 0; start < goalPart.size(); start++) {
            for (int end = start + 2; end < goalPart.size() + 1; end++) {
                if (start == 0 && end == goalPart.size()) {
                    continue;
                }
                CompositePart part = goalPart.subpart(start, end);

                if (_requiredParts.contains(part)) {
