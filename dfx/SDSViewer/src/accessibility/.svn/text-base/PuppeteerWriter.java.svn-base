...
public class PuppeteerWriter {
    public HashMap<SDSBinaryTree, String> _names = new HashMap<SDSBinaryTree, String>();
    public ArrayList<ArrayList<SDSBinaryTree>> stages = new ArrayList<ArrayList<SDSBinaryTree>>();
    ...
    public void traverseTree(SDSBinaryTree tree, int stage) {
        if (tree.getLeft() == null && tree.getRight() == null) {
            return;
        }

        if (stages.size() <= stage) {
            stages.add(new ArrayList<SDSBinaryTree>());
        }
        ...
        System.out.println(Integer.toString(stage) + " " + tree.getNode().getPart().toString());
    }

    public String getRdString(ArrayList<SDSBinaryTree> stage) {
        String result = "";

        for (int index = 0; index < stage.size(); index++) {
            SDSBinaryTree currentTree = stage.get(index);
            SDSBinaryTree leftTree = currentTree.getLeft();
            SDSBinaryTree rightTree = currentTree.getRight();
            String leftName = leftTree.getNode().getPart().toString();
            String rightName = rightTree.getNode().getPart().toString();
            if (!_names.containsKey(leftTree)) {
                _names.put(leftTree, leftTree.getNode().getPart().toString());
            } else {
               leftName = _names.get(leftTree);
            }
            if (!_names.containsKey(rightTree)) {
                _names.put(rightTree, rightTree.getNode().getPart().toString());
            } else {
    ...
    public String getLgString(ArrayList<SDSBinaryTree> stage) {
        String result = "";
    ...
    public void writeToFile(SDSBinaryTree tree, String fileName) {
        String result = "";
        traverseTree(tree, 0);
