 ...
 */
public class JungSugiyamaCenter<V, E> extends AbstractLayout<V, E> {
    ...
    public JungSugiyamaCenter(Graph<V, E> g) {
        this(g, DEFAULT_ORIENTATION, DEFAULT_HORIZONTAL_SPACING, DEFAULT_VERTICAL_SPACING);
    }

    public JungSugiyamaCenter(Graph<V, E> g, Orientation orientation, int horzSpacing, int vertSpacing) {
        super(g);
    ...
    public void initialize() {
        if (!executed) {
            ...
            for (LinkedList<CellWrapper<V>> level : graphLevels) {
                ...
                max = max > level.size() ? max : level.size();
                }
            }
            for (LinkedList<CellWrapper<V>> level : graphLevels) {
                int min = 100000;
                for (CellWrapper<V> wrapper : level) {
                    ...
                    min = min < wrapper.gridPosition ? min : wrapper.gridPosition;

                    if (orientation.equals(Orientation.TOP)) {
                        double xCoordinate = 10.0 + (max - level.size()) * horzSpacing / 2 + ((wrapper.gridPosition - min) * horzSpacing);
